...
func NewRabbitBroker(rabbitMQConfig RabbitConfig, dispatcher bus.Dispachable) (interfaces.Broker, error) {
	...
	if err := declareExchanges(firstCh, rabbitMQConfig.ExchangeType, rabbitMQConfig.ChatExchange, rabbitMQConfig.BoardExchange); err != nil {
		return nil, err
	}
	...
	for {
		...
		case msg, ok := <-msgs:
			...
			go func(msg amqp.Delivery) {
				...
				if err := r.dispatcher.HandleEvent(context.Background(), pattern, data, nil); err != nil {
					msg.Nack(false, true)
					return
				}
