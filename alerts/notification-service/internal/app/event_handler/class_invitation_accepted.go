...
type ClassInvitationAcceptedHandler struct {
	publisher interfaces.NotificationManager
	repo      repository.NotificationRepository
}

func NewClassInvitationAcceptedHandler(publisher interfaces.NotificationManager,
	repo repository.NotificationRepository) interfaces.EventHandler {
	return &ClassInvitationAcceptedHandler{repo: repo, publisher: publisher}
}

func (c *ClassInvitationAcceptedHandler) Handle(ctx context.Context, body []byte) error {
	newEvent := classinvitation.ClassInvitationAcceptedEvent{}
